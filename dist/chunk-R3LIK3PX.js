...
var buildSrcSet = (src, width, height, densities, quality, format, buildUrl) => Array.from(new Set(densities)).filter((d) => d > 0 && Number.isFinite(d)).sort((a, b) => a - b).map((d) => `${buildUrl(src, { width: width * d, height: height * d, quality, format })} ${d}x`).join(", ");
