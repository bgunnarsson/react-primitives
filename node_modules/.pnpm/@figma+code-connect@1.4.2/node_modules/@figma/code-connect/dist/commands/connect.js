...
function addConnectCommandToProgram(program) {
    ...
    addBaseCommand(connectCommand, 'migrate', 'Parse Code Connect files and migrate their templates into .figma.js files that can be published directly without parsing.')
        ...
        .option('--typescript', 'output migrated templates as TypeScript (.figma.ts) instead of JavaScript (.figma.js)')
    ...
    for (const [figmaUrl, group] of Object.entries(codeConnectObjectsByFigmaUrl)) {
        try {
            ...
            const { outputPath, skipped } = hasVariants
                ? (0, migration_helpers_1.writeVariantTemplateFile)(group, figmaUrl, cmd.outDir, dir, localSourcePath, filePathsCreated, useTypeScript)
                : (0, migration_helpers_1.writeTemplateFile)(representativeDoc, cmd.outDir, dir, localSourcePath, filePathsCreated, cmd.removeProps, useTypeScript);
            if (skipped) {
