...
const panelBase: React.CSSProperties = { position: 'fixed', background: 'white', padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }
...
function StorySheet({ side }: { side: 'right' | 'left' | 'bottom' | 'top' }) {
  ...
  return (
    <>
      <button style={triggerStyle} onClick={() => setOpen(true)}>Open ({side})</button>
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side={side} style={sides[side]} overlayStyle={overlayStyle}>
          ...
          <SheetDescription style={{ margin: 0, fontSize: 14, color: '#6b7280' }}>Sheet description goes here.</SheetDescription>
